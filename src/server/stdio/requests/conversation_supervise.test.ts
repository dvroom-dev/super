...
describe("handleConversationSupervise", () => {
  ...
  it.serial("applies certify_wrapup continue decisions and persists transition payload without a mode switch", async () => {
    const workspaceRoot = await makeTempRoot("conv-supervise-certify-wrapup-");
    await fs.mkdir(path.join(workspaceRoot, ".ai-supervisor"), { recursive: true });
    await fs.writeFile(
      path.join(workspaceRoot, ".ai-supervisor", "config.yaml"),
      [
    ...
    const { ctx } = makeCtx({ conversationId: "conversation_certify_wrapup" });
    process.env.MOCK_PROVIDER_STREAMED_TEXT = [
      "```tool_call name=certify_wrapup",
      '{"wrapup_level":2,"reason":"level 2 wrap-up complete","user_message":"advance to frontier"}',
      "```",
    ].join("\n");
    process.env.MOCK_PROVIDER_RUNONCE_TEXT = JSON.stringify({
      decision: "continue",
      ...
      mode_assessment: {
        current_mode_stop_satisfied: false,
        candidate_modes_ranked: [
          { mode: "theory", confidence: "high", evidence: "stay in theory while release happens" },
        ],
        recommended_action: "continue",
      },
      reasoning: "certify wrap-up without requiring a same-mode switch",
      agent_model: null,
    });
    try {
      const result = await handleConversationSupervise(ctx, {
        ...
        docPath: path.join(workspaceRoot, "session.md"),
        documentText: makeModeDoc({
          conversationId: "conversation_certify_wrapup",
          forkId: "fork_doc",
          mode: "theory",
        }),
        models: ["mock-model"],
        ...
        supervisorModel: "mock-supervisor",
        cycleLimit: 1,
        supervisor: { enabled: true, stop_condition: "task complete" },
      });
      expect(result.activeMode).toBe("theory");
      expect(result.activeTransitionPayload).toEqual({
        wrapup_certified: "true",
